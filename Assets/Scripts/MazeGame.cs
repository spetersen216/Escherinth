...
public class MazeGame : MonoBehaviour {
	...
	void Start () {
		...
		cell.Init(new Point3(0,0,0),cellFloor,cellWalls[0], cellWallTops[0], cellFloorMat, cellWallMat, cellWallTopMat,
		                                      lightFlicker, new Vector3(1,1,1), new Vector3(1,1,1), new Vector3(1,1,1), new Vector3(1,1,1));
		cells = new MazeCell[2,2,2];
		cells [0, 0, 0] = cell;
		lights.Init(mazeStruct,cells);
		door = mazeStruct.GetDoor();
		monster = (GameObject)Instantiate(monster.gameObject, new Vector3 (1, 1.11f, 1), Quaternion.identity);
